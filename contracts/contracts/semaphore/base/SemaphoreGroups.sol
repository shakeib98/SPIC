...
abstract contract SemaphoreGroups is Context, ISemaphoreGroups {
    ...
    mapping(uint256 => IncrementalTreeData) internal votersGroup;
    ...
    function _createGroup(
        ...
        emit GroupCreated(groupId, depth, zeroValue); //Same for voters group as well
    }
    ...
    function _addVote(uint256 groupId, uint256 externalNullifier) internal virtual {
        require(getDepth(groupId) != 0, "SemaphoreGroups: group does not exist");
        ...
        emit VoteAddedToRoot(groupId, externalNullifier, root);
    }
    ...
    function getRoot(uint256 groupId) public view virtual override returns (uint256, uint256) {
        return (groups[groupId].root, votersGroup[groupId].root);
    }

    /// @dev See {ISemaphoreGroups-getDepth}.
    function getDepth(uint256 groupId) public view virtual override returns (uint8) { //Depth of both mapping will be same
        return groups[groupId].depth;
    }

    /// @dev See {ISemaphoreGroups-getNumberOfLeaves}.
    function getNumberOfLeaves(uint256 groupId) public view virtual override returns (uint256) { //no of leaves of both mapping will be same
